...
function checkFormValidity() {
    const isVideoNameFilled = videoNameInput.value.trim() !== '';

    if (isImageLoaded && isVideoNameFilled) {
...
videoNameInput.addEventListener('input', checkFormValidity);
...
deleteBtn.addEventListener('click', () => {
    ...
    .then(data => {
        if (data.success) {
            ...
            alert('Ошибка при удалении изображения: ' + data.error);
        }
    })
    .catch(error => {
        console.error('Ошибка:', error);
...
continueBtn.addEventListener('click', () => {
    ...
    fetch('/glavnoe_continue_processing', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    .then(result => {
        hideLoadingOverlay();
        if (result.success) {
            console.log('Файл успешно сохранен во временную папку:', result.temp_path);
            // Переходим на страницу анимации с параметрами
            window.location.href = `/glavnoe_animate?file_path=${encodeURIComponent(result.temp_path)}&video_name=${encodeURIComponent(videoName)}`;
        } else {
